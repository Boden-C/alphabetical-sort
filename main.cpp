...
void countingSort(string array[], int length, size_t characterAt){
  std::unordered_map<string, int> codeFor = {
    ...
    for (int i = 0; i <length; i++){ //counts each unique character code
      arrayC[characterAt < array[i].size() /*if character exists*/ ? (int)(unsigned char)array[i][characterAt] + 2 /*find character code*/ : 0 /*code of 0*/]++;
    }
    for (int i = 1; i <127; i++){ //finds index
      arrayC[i] += arrayC[i - 1];
    }
    for (int i = length - 2; i >= 0; i--){ //creates new array using new index
              newArray[
