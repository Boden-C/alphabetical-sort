...
void countingSort(std::vector<std::string> &array, int length, size_t characterAt){
  std::unordered_map<std::string, int> codeFor = {
  ...
  std::vector<std::string> newArray(length);
    std::vector<int> arrayOfCount(27, 0);
    std::vector<int> indexes(length);
    
    for (int i = 0; i < length; i++){ //counts each unique character code
      int index = characterAt < array[i].size() ? (codeFor.find(std::string(1,array[i][characterAt])) == codeFor.end() ? 0 : codeFor.at(std::string(1,array[i][characterAt]))) : 0;
	arrayOfCount[index]++;
	indexes[i] = index;
    }
...
void radixSort(std::vector<std::string> &unsortedArray, int length){
